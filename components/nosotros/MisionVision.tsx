...
const items = [
  ...
  },
  {
    ...
    accent: 'border-chart-2/20 hover:border-chart-2/40',
  },
  {
    icon: Compass,
    label: 'Propósito',
    ...
    accent: 'border-violet-200 hover:border-violet-400/40',
  },
]
          ...
          {items.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group rounded-2xl border bg-white shadow-sm ${item.accent} p-6 md:p-8 transition-all duration-300 hover:shadow-xl hover:-translate-y-1`}
              >
                ...
                <span className={`text-xs font-bold uppercase tracking-widest ${item.iconColor} mb-2 block`}>
                  {item.label}
                </span>
