    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-7">
      ...
      <div className="relative overflow-hidden rounded-2xl p-7 bg-gradient-to-br from-rose-600 to-pink-600 shadow-lg shadow-rose-200">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-rose-200 mb-1">Panel Admin</p>
            <h1 className="text-3xl font-bold text-white tracking-tight">Analytics</h1>
            <p className="text-rose-200 mt-1 text-sm">Estadísticas de visitas del sitio web</p>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {[
          { label: `Visitas (${days}d)`, value: data?.totalViews ?? '—', icon: Eye,       bg: 'bg-rose-50 border-rose-100',   ic: 'text-rose-500',   ib: 'bg-rose-100' },
          { label: 'Promedio diario',    value: avgDaily || '—',         icon: TrendingUp, bg: 'bg-violet-50 border-violet-100', ic: 'text-violet-500', ib: 'bg-violet-100' },
          { label: 'Páginas únicas',     value: data?.topPages.length ?? '—', icon: Globe, bg: 'bg-blue-50 border-blue-100',   ic: 'text-blue-500',   ib: 'bg-blue-100' },
        ].map((s, i) => (
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-3 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Últimos {days} días</p>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data?.dailyViews || []}>
                  ...
                  <Area type="monotone" dataKey="count" stroke="hsl(var(--primary))" strokeWidth={2}
                    fill="url(#viewsGrad)" name="Visitas" />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-semibold text-slate-900">Páginas más visitadas</h3>
          ...
          {loading ? (
            <div className="space-y-3">
              {[...Array(5)].map((_, i) => <div key={i} className="h-8 bg-slate-50 rounded-lg animate-pulse" />)}
            </div>
          ) : (
                  ...
                  <div key={p.path} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs font-medium text-slate-700 truncate">{p.path || '/'}</span>
